const searchBar = document.getElementById("search-bar"); // Get element for search bar
const searchBarValue = document.getElementById("search-value"); // Get element for search bar input
const searchBarMealsList = document.createElement("ul"); // Creating element for meals list
const searchInput = window.location.search; // Searches for a search in the given window
const mealCardsList = document.getElementById("meals-list"); // Get element for every meal in the list
const searchBtn = document.getElementById("search-btn"); // Get element for search button
const mealCardsWrapper = document.getElementById("meal-cards-wrapper"); // Get element for meal wrapper
...
const buttonClick = function (a, url) {
    ...
    for (elem of meals) {
        if(elem["idMeal"] == a) {
          // Sets the meal description in the localStorage.
            localStorage.setItem("mealsDesc", JSON.stringify(elem));
        }
    }
};

// Array for storing the favourite meals
...
const addToFavourites = function(id) {
    ...
    // Fetch data from SEARCH_MEAL_API_URL
...
// Empty the search bar list when clicked outside the search box
...
// Event Listener for dynamic input
searchBarValue.addEventListener("keyup", (e) => {
  while (searchBarMealsList.firstChild) {
    searchBarMealsList.removeChild(searchBarMealsList.firstChild);
  }

  // Passing th url for accessing the meals from the database  
